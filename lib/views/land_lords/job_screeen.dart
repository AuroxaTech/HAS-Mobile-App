              ...
              child: Column(
                  children: [
                      ...
                      children: [
                        ...
                        Expanded(
                          child: topContainer(
                            ...
                            text: "Completed(${controller.completedJobController.itemList?.length ?? 0})",
                            textColor: controller.completed.value ? primaryColor : greyColor,
                            borderColor: controller.completed.value ? primaryColor : greyColor,
                          ),
                        ),
                        w10,
                        Expanded(
                          child: topContainer(
                            ...
                            text: "Rejected(${controller.rejectedJobController.itemList?.length ?? 0})",
                            textColor: controller.reject.value ? primaryColor : greyColor,
                            borderColor:  controller.reject.value ? primaryColor : greyColor,
                          ),
                        ),
                      ],
                    ),
                   ...
                   // jobWidget(context, ),
                    ...
                    controller.pending.value ? PagedListView<int, PendingJob>(
                      ...
                      builderDelegate: PagedChildBuilderDelegate<PendingJob>(
                          ...
                          itemBuilder: (context, item, index) {
                            ...
                            String imagesString =  item.request == null ? "" : item.request.service.media;
                            ...
                            return Column(
                              children: [
                                jobWidget(
                                    ...
                                    title: item.request.service.serviceName , // Using null-aware operators to avoid runtime errors
                                    contactDetail: item.provider.email,
                                    clientName: item.provider.fullname,
                    ...
                    controller.completed.value ? PagedListView<int, CompletedJob>(
                      shrinkWrap: true,
                      pagingController: controller.completedJobController,
                      physics: const BouncingScrollPhysics(),
                      builderDelegate: PagedChildBuilderDelegate<CompletedJob>(
                          firstPageErrorIndicatorBuilder: (context) => MaterialButton(
                            child: const Text("No Data Found, Tap to try again."),
                            onPressed: () => controller.completedJobController.refresh(),
                          ),
                          newPageErrorIndicatorBuilder: (context) => MaterialButton(
                            child: const Text("Failed to load more items. Tap to try again."),
                            onPressed: () => controller.completedJobController.retryLastFailedRequest(),
                          ),
                          itemBuilder: (context, item, index) {
                            DateTime createdAt = item.createdAt;
                            String requestDate = DateFormat('dd-M-yy').format(createdAt); // Adjust the pattern as needed
                    ...
                    controller.reject.value ? PagedListView<int, RejectedJob>(
                      shrinkWrap: true,
                      pagingController: controller.rejectedJobController,
                      physics: const BouncingScrollPhysics(),
                      builderDelegate: PagedChildBuilderDelegate<RejectedJob>(
                          firstPageErrorIndicatorBuilder: (context) => MaterialButton(
                            child: const Text("No Data Found, Tap to try again."),
                            onPressed: () => controller.rejectedJobController.refresh(),
                          ),
                          newPageErrorIndicatorBuilder: (context) => MaterialButton(
                            child: const Text("Failed to load more items. Tap to try again."),
                            onPressed: () => controller.rejectedJobController.retryLastFailedRequest(),
                          ),
                          itemBuilder: (context, item, index) {
                            DateTime createdAt = item.createdAt;
                            String requestDate = DateFormat('dd-M-yy').format(createdAt); // Adjust the pattern as needed
